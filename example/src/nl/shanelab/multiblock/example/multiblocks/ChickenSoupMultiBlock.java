...
public class ChickenSoupMultiBlock implements IMultiBlock {
	...
	public void onActivate(Plugin plugin, Location location, Player player, MultiBlockActivationType activationType) {
		if (activationType == MultiBlockActivationType.CORE_INTERACTED) {
			if (player.getInventory().getItemInMainHand().getType() == Material.STICK) {
				if (location.getBlock().getType() == Material.CAULDRON) {
					...
					player.sendMessage(location.getBlock().getClass().getSimpleName());
				}
			}
		}
	}
	...
	private ItemStack buildChickenSoup() {
		...
		lore.add(ChatColor.BLUE + "This is absolutely real chicken soup");
